-- =====================================================
-- APE IN PVP MODE - ACTION RPC COMPAT PATCH
-- =====================================================
-- Accept match_status = 'active' in addition to 'in_progress'
-- Accept phase = 'TURN_START' as equivalent to 'DRAW' for first draw.
--
-- This keeps the action RPCs compatible with ape_in_pvp_join_match_v1
-- which flips match_status to 'active' and may set phase to 'TURN_START'.

-- Draw: allow TURN_START + active
...
BEGIN
  ...
  -- (same pool + draw logic as ape_in_pvp_action_draw in 20260130150000...)
  v_cipher1 := ARRAY[
...
-- Roll/Stack/Forfeit: allow active as in-progress
-- (Implementation detail: we only need to relax the match_status checks; the rest is unchanged.)
...
BEGIN
  ...
  IF v_is_bust THEN
    ...
    END IF;
    UPDATE public.ape_in_pvp_matches SET game_state = v_state, last_action_at = v_now WHERE id = p_match_id;
    RETURN QUERY SELECT m.id, m.match_status, m.winner_id, m.forfeited_by, m.last_action_at, m.game_state FROM public.ape_in_pvp_matches m WHERE m.id = p_match_id;
    RETURN;
  END IF;
...
DECLARE
  ...
  v_next_seat text;
BEGIN
  SELECT * INTO v_match FROM public.ape_in_pvp_matches WHERE id = p_match_id FOR UPDATE;
  ...
  IF v_new_total >= 150 THEN
    ...
    UPDATE public.ape_in_pvp_matches
    SET game_state = v_state, last_action_at = v_now, match_status = 'completed', winner_id = p_actor_user_id, ended_at = v_now
    WHERE id = p_match_id;
  ELSE
    v_next_seat := CASE WHEN v_current_turn_seat='seat1' THEN 'seat2' ELSE 'seat1' END;
    v_state := jsonb_set(v_state, '{current_turn_seat}', to_jsonb(v_next_seat), true);
