-- =====================================================
-- APE IN PVP MODE - NAMESPACED RPCs (table-aligned)
-- =====================================================
-- Problem this solves:
-- - Generic PvP systems may use public.pvp_matches + pvp_action_*
-- - Ape In PvP uses public.ape_in_pvp_matches
-- This migration creates Ape-In-specific RPC names that operate on
-- public.ape_in_pvp_matches so the Ape In frontend never calls into the
-- wrong "universe".
--
-- These functions intentionally mirror the Sandy-parity behavior from:
-- `20260130120000_ape_in_pvp_phase3_sandy_parity_actions.sql`
--
-- NOTE: This does NOT delete or alter any existing pvp_* functions.

-- ---------------------------------------------------------
-- Build initial game_state v1 (Sandy-parity PvP schema)
-- ---------------------------------------------------------
CREATE OR REPLACE FUNCTION public.ape_in_pvp_build_initial_game_state_v1(
  p_player1_id uuid,
  p_player2_id uuid,
...
GRANT EXECUTE ON FUNCTION public.ape_in_pvp_build_initial_game_state_v1(uuid, uuid, integer) TO service_role;
GRANT EXECUTE ON FUNCTION public.ape_in_pvp_build_initial_game_state_v1(uuid, uuid, integer) TO authenticated;

-- ---------------------------------------
-- Ensure game_state initialized to v1
-- ---------------------------------------
CREATE OR REPLACE FUNCTION public.ape_in_pvp_ensure_game_state_v1(p_match_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = 'pg_catalog, public'
...
BEGIN
  ...
  v_state := public.ape_in_pvp_build_initial_game_state_v1(v_match.player1_id, v_match.player2_id, v_match.first_turn_player);
...
GRANT EXECUTE ON FUNCTION public.ape_in_pvp_ensure_game_state_v1(uuid) TO service_role;
GRANT EXECUTE ON FUNCTION public.ape_in_pvp_ensure_game_state_v1(uuid) TO authenticated;

-- ======================================================
-- Phase 3: Draw action (Sandy-parity deck + Ape In)
-- ======================================================
CREATE OR REPLACE FUNCTION public.ape_in_pvp_action_draw(
  p_match_id uuid,
...
BEGIN
  ...
  v_state := public.ape_in_pvp_ensure_game_state_v1(p_match_id);
...
GRANT EXECUTE ON FUNCTION public.ape_in_pvp_action_draw(uuid, uuid) TO service_role;
GRANT EXECUTE ON FUNCTION public.ape_in_pvp_action_draw(uuid, uuid) TO authenticated;

-- ======================================================
-- Phase 3: Roll action (Sandy-parity bust/bearish/ApeIn)
-- ======================================================
CREATE OR REPLACE FUNCTION public.ape_in_pvp_action_roll(
  p_match_id uuid,
  p_actor_user_id uuid
)
RETURNS TABLE (
...
GRANT EXECUTE ON FUNCTION public.ape_in_pvp_action_roll(uuid, uuid) TO service_role;
GRANT EXECUTE ON FUNCTION public.ape_in_pvp_action_roll(uuid, uuid) TO authenticated;

-- ======================================================
-- Phase 3: Stack action (bank turn -> total, 150 wins)
-- ======================================================
CREATE OR REPLACE FUNCTION public.ape_in_pvp_action_stack(
  p_match_id uuid,
  p_actor_user_id uuid
)
RETURNS TABLE (
...
GRANT EXECUTE ON FUNCTION public.ape_in_pvp_action_stack(uuid, uuid) TO service_role;
GRANT EXECUTE ON FUNCTION public.ape_in_pvp_action_stack(uuid, uuid) TO authenticated;

-- ======================================================
-- Phase 3: Forfeit action (unchanged)
-- ======================================================
CREATE OR REPLACE FUNCTION public.ape_in_pvp_action_forfeit(
  p_match_id uuid,
  p_actor_user_id uuid
)
RETURNS TABLE (
...
GRANT EXECUTE ON FUNCTION public.ape_in_pvp_action_forfeit(uuid, uuid) TO service_role;
GRANT EXECUTE ON FUNCTION public.ape_in_pvp_action_forfeit(uuid, uuid) TO authenticated;
