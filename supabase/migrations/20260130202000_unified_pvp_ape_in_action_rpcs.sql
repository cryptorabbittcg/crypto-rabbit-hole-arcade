-- =====================================================
-- UNIFIED PVP (pvp_matches) - APE IN ACTION RPCs (Sandy-parity)
-- =====================================================
-- Option A: Ape In PvP uses public.pvp_matches with game_code='ape_in'.
-- These RPCs implement the same Sandy-parity gameplay rules, but operate on pvp_matches.
--
-- Functions:
-- - pvp_ensure_game_state_v1
-- - pvp_action_draw
-- - pvp_action_roll
-- - pvp_action_stack
-- - pvp_action_forfeit
...
BEGIN
  ...
  IF v_is_bust THEN
    ...
    SET game_state = v_state,
        ...
        match_status = CASE WHEN match_status = 'active' THEN 'in_progress' ELSE match_status END
    WHERE id = p_match_id;
    RETURN QUERY SELECT m.id, m.match_status, m.winner_id, m.forfeited_by, m.last_action_at, m.game_state FROM public.pvp_matches m WHERE m.id = p_match_id;
    RETURN;
  END IF;
...
-- ======================================================
-- Stack action (bank sats, win at 150)
-- ======================================================
