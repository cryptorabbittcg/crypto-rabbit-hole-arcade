...
import { verifyAndCompleteIntent } from "@/lib/payments/apeInPlaysPayment"
 ...
 * 
 * Verifies on-chain transaction and grants plays.
 * 
 * Request body: { address: string, intentId: string, txHash: string }
 * Response: { success: boolean, playsAdded?: number, purchasedPlaysRemaining?: number, error?: string }
 */
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      playsAdded: result.playsAdded,
      purchasedPlaysRemaining: result.purchasedPlaysRemaining,
