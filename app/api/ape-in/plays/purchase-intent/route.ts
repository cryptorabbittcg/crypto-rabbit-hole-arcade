...
import { createIntent } from "@/lib/payments/apeInPlaysPayment"
 ...
 * 
 * Creates a purchase intent for play purchase with transaction verification.
 * Returns intent details for client to send transaction.
 * 
 * Request body: { address: string, playsAmount?: number, gameMode?: string }
 * Response: { intentId, chainId, recipient, priceWei, playsAmount, expiresAt }
 */
export async function POST(request: NextRequest) {
  try {
    const body = await request.json()
    const { address, playsAmount, gameMode } = body
    ...
    if (error.message?.includes("Only 5 plays bundle")) {
      return NextResponse.json(
        { error: error.message },
        { status: 400 }
      )
